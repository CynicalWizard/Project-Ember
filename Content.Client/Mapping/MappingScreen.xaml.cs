...
public sealed partial class MappingScreen : InGameScreen
{
    ...
    public MappingScreen()
    {
        ...
        EmberPaintPreset.OnItemSelected += args => EmberPaintPreset.SelectId(args.Id);
        
        var presets = IoCManager.Resolve<IPrototypeManager>().EnumeratePrototypes<EmberPaintPresetPrototype>().ToList();
        for (var i = 0; i < presets.Count; i++)
        {
            EmberPaintPreset.AddItem(presets[i].Name, i);
            EmberPaintPreset.SetItemMetadata(i, presets[i].ID);
        }
        
        if (presets.Count > 0)
            EmberPaintPreset.SelectId(0);
    }

    public string SelectedEmberPaintMode => _emberPaintModes[EmberPaintMode.SelectedId];

    public string? SelectedEmberPaintPreset
    {
        get
        {
            if (EmberPaintPreset.ItemCount == 0)
                return null;
            return (string) EmberPaintPreset.GetItemMetadata(EmberPaintPreset.SelectedId)!;
        }
    }
