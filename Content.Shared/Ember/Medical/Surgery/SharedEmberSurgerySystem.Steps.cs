...
public abstract partial class SharedEmberSurgerySystem
{
    ...
    {
        SubscribeLocalEvent<EmberSurgeryStepComponent, EmberSurgeryStepEvent>(OnToolStep);
        SubscribeLocalEvent<EmberSurgeryStepComponent, EmberSurgeryStepCompleteCheckEvent>(OnToolCheck);
        SubscribeLocalEvent<EmberSurgeryStepComponent, EmberSurgeryCanPerformStepEvent>(OnToolCanPerform);

        //SubSurgery<EmberSurgeryCutLarvaRootsStepComponent>(OnCutLarvaRootsStep, OnCutLarvaRootsCheck);
        ...
        SubSurgery<EmberSurgeryTendWoundsEffectComponent>(OnTendWoundsStep, OnTendWoundsCheck);
        SubSurgery<EmberSurgeryStepCavityEffectComponent>(OnCavityStep, OnCavityCheck);
        SubSurgery<EmberSurgeryAddPartStepComponent>(OnAddPartStep, OnAddPartCheck);
        SubSurgery<EmberSurgeryAffixPartStepComponent>(OnAffixPartStep, OnAffixPartCheck);
        SubSurgery<EmberSurgeryRemovePartStepComponent>(OnRemovePartStep, OnRemovePartCheck);
        SubSurgery<EmberSurgeryAddOrganStepComponent>(OnAddOrganStep, OnAddOrganCheck);
        SubSurgery<EmberSurgeryRemoveOrganStepComponent>(OnRemoveOrganStep, OnRemoveOrganCheck);
        SubSurgery<EmberSurgeryAffixOrganStepComponent>(OnAffixOrganStep, OnAffixOrganCheck);
        SubSurgery<EmberSurgeryAddMarkingStepComponent>(OnAddMarkingStep, OnAddMarkingCheck);
        SubSurgery<EmberSurgeryRemoveMarkingStepComponent>(OnRemoveMarkingStep, OnRemoveMarkingCheck);
        Subs.BuiEvents<EmberSurgeryTargetComponent>(EmberSurgeryUIKey.Key, subs =>
        {
            subs.Event<EmberSurgeryStepChosenBuiMsg>(OnSurgeryTargetStepChosen);
        });
    }

    private void SubSurgery<TComp>(EntityEventRefHandler<TComp, EmberSurgeryStepEvent> onStep,
        EntityEventRefHandler<TComp, EmberSurgeryStepCompleteCheckEvent> onComplete) where TComp : IComponent
    {
        SubscribeLocalEvent(onStep);
        SubscribeLocalEvent(onComplete);
    }

    private void OnToolStep(Entity<EmberSurgeryStepComponent> ent, ref EmberSurgeryStepEvent args)
    {
        if (ent.Comp.Tool != null)
        {
            foreach (var reg in ent.Comp.Tool.Values)
            {
                ...
                if (_net.IsServer &&
                    TryComp(tool, out EmberSurgeryToolComponent? toolComp) &&
        ...
        {
            if (!HasComp<EmberSanitizedComponent>(args.User))
            {
                // Zeta - Xelthia Glove Jacket Check
                _inventory.TryGetSlotEntity(args.User, "outerClothing", out var glovejacket);
                if (!HasComp<EmberGloveJacketComponent>(glovejacket) || !_inventory.TryGetSlotEntity(args.User, "mask", out var _))
                {
                    var sepsis = new DamageSpecifier(_prototypes.Index<DamageTypePrototype>("Poison"), 5);
                    var ev = new EmberSurgeryStepDamageEvent(args.User, args.Body, args.Part, args.Surgery, sepsis, 0.5f);
    ...
    private void OnToolCheck(Entity<EmberSurgeryStepComponent> ent, ref EmberSurgeryStepCompleteCheckEvent args)
    {
    ...
    private void OnToolCanPerform(Entity<EmberSurgeryStepComponent> ent, ref EmberSurgeryCanPerformStepEvent args)
    {
        if (HasComp<EmberSurgeryOperatingTableConditionComponent>(ent))
        {
            if (!TryComp(args.Body, out BuckleComponent? buckle) ||
                !HasComp<EmberOperatingTableComponent>(buckle.BuckledTo))
            {
                args.Invalid = EmberStepInvalidReason.NeedsOperatingTable;
                return;
            }
        }
        ...
        {
            if (HasComp<EmberSurgeryIgnoreClothingComponent>(args.User))
                return;
            ...
            while (containerSlotEnumerator.MoveNext(out var containerSlot))
            {
                ...
                args.Invalid = EmberStepInvalidReason.Armor;
        ...
        if (args.Invalid != EmberStepInvalidReason.None)
            return;
        ...
        if (ent.Comp.Tool != null)
        {
            ...
            foreach (var reg in ent.Comp.Tool.Values)
            {
                ...
                {
                    args.Invalid = EmberStepInvalidReason.MissingTool;

                    if (reg.Component is IEmberSurgeryToolComponent required)
    ...
    private void OnTendWoundsStep(Entity<EmberSurgeryTendWoundsEffectComponent> ent, ref EmberSurgeryStepEvent args)
    {
        ...
        var ev = new EmberSurgeryStepDamageEvent(args.User, args.Body, args.Part, args.Surgery, adjustedDamage, 0.5f);
        RaiseLocalEvent(args.Body, ref ev);
    }

    private void OnTendWoundsCheck(Entity<EmberSurgeryTendWoundsEffectComponent> ent, ref EmberSurgeryStepCompleteCheckEvent args)
    {
        var group = ent.Comp.MainGroup == "Brute" ? BruteDamageTypes : BurnDamageTypes;
    ...
    /*private void OnCutLarvaRootsStep(Entity<EmberSurgeryCutLarvaRootsStepComponent> ent, ref EmberSurgeryStepEvent args)
    {
    ...
    private void OnCutLarvaRootsCheck(Entity<EmberSurgeryCutLarvaRootsStepComponent> ent, ref EmberSurgeryStepCompleteCheckEvent args)
    {
    ...
    private void OnCavityStep(Entity<EmberSurgeryStepCavityEffectComponent> ent, ref EmberSurgeryStepEvent args)
    {
    ...
    private void OnCavityCheck(Entity<EmberSurgeryStepCavityEffectComponent> ent, ref EmberSurgeryStepCompleteCheckEvent args)
    {
    ...
    private void OnAddPartStep(Entity<EmberSurgeryAddPartStepComponent> ent, ref EmberSurgeryStepEvent args)
    {
        if (!TryComp(args.Surgery, out EmberSurgeryPartRemovedConditionComponent? removedComp))
            return;
        ...
        foreach (var tool in args.Tools)
        {
                ...
                && (removedComp.Symmetry == null || partComp.Symmetry == removedComp.Symmetry))
            {
                ...
                EnsureComp<EmberBodyPartReattachedComponent>(tool);
    ...
    private void OnAffixPartStep(Entity<EmberSurgeryAffixPartStepComponent> ent, ref EmberSurgeryStepEvent args)
    {
        if (!TryComp(args.Surgery, out EmberSurgeryPartRemovedConditionComponent? removedComp))
            return;
        ...
        if (targetPart != default)
        {
            ...
            RemComp<EmberBodyPartReattachedComponent>(targetPart.Id);
        }
    }

    private void OnAffixPartCheck(Entity<EmberSurgeryAffixPartStepComponent> ent, ref EmberSurgeryStepCompleteCheckEvent args)
    {
        if (!TryComp(args.Surgery, out EmberSurgeryPartRemovedConditionComponent? removedComp))
            return;
        ...
        if (targetPart != default
            && HasComp<EmberBodyPartReattachedComponent>(targetPart.Id))
            args.Cancelled = true;
    }

    private void OnAddPartCheck(Entity<EmberSurgeryAddPartStepComponent> ent, ref EmberSurgeryStepCompleteCheckEvent args)
    {
        if (!TryComp(args.Surgery, out EmberSurgeryPartRemovedConditionComponent? removedComp)
    ...
    private void OnRemovePartStep(Entity<EmberSurgeryRemovePartStepComponent> ent, ref EmberSurgeryStepEvent args)
    {
        if (!TryComp(args.Part, out BodyPartComponent? partComp)
    ...
    private void OnRemovePartCheck(Entity<EmberSurgeryRemovePartStepComponent> ent, ref EmberSurgeryStepCompleteCheckEvent args)
    {
        if (!TryComp(args.Part, out BodyPartComponent? partComp)
            || partComp.Body == args.Body)
            args.Cancelled = true;
    ...
    private void OnAddOrganStep(Entity<EmberSurgeryAddOrganStepComponent> ent, ref EmberSurgeryStepEvent args)
    {
        if (!TryComp(args.Part, out BodyPartComponent? partComp)
            || partComp.Body != args.Body
            || !TryComp(args.Surgery, out EmberSurgeryOrganConditionComponent? organComp)
        ...
        foreach (var tool in args.Tools)
        {
            ...
            {
                EnsureComp<EmberOrganReattachedComponent>(tool);
                if (_body.TrySetOrganUsed(tool, true, insertedOrgan)
                    && insertedOrgan.OriginalBody != args.Body)
                {
                    var ev = new EmberSurgeryStepDamageChangeEvent(args.User, args.Body, args.Part, ent);
    ...
    private void OnAddOrganCheck(Entity<EmberSurgeryAddOrganStepComponent> ent, ref EmberSurgeryStepCompleteCheckEvent args)
    {
        if (!TryComp<EmberSurgeryOrganConditionComponent>(args.Surgery, out var organComp)
    ...
    private void OnAffixOrganStep(Entity<EmberSurgeryAffixOrganStepComponent> ent, ref EmberSurgeryStepEvent args)
    {
        if (!TryComp(args.Surgery, out EmberSurgeryOrganConditionComponent? removedOrganComp)
        ...
        foreach (var reg in removedOrganComp.Organ.Values)
        {
            ...
            if (organs != null && organs.Count > 0)
                RemComp<EmberOrganReattachedComponent>(organs[0].Id);
        }

    }

    private void OnAffixOrganCheck(Entity<EmberSurgeryAffixOrganStepComponent> ent, ref EmberSurgeryStepCompleteCheckEvent args)
    {
        if (!TryComp(args.Surgery, out EmberSurgeryOrganConditionComponent? removedOrganComp)
            || removedOrganComp.Organ == null
            || !removedOrganComp.Reattaching)
    ...
    private void OnRemoveOrganStep(Entity<EmberSurgeryRemoveOrganStepComponent> ent, ref EmberSurgeryStepEvent args)
    {
        if (!TryComp<EmberSurgeryOrganConditionComponent>(args.Surgery, out var organComp)
            || organComp.Organ == null)
            return;
    ...
    private void OnRemoveOrganCheck(Entity<EmberSurgeryRemoveOrganStepComponent> ent, ref EmberSurgeryStepCompleteCheckEvent args)
    {
        if (!TryComp<EmberSurgeryOrganConditionComponent>(args.Surgery, out var organComp)
            || organComp.Organ == null
            || !TryComp(args.Part, out BodyPartComponent? partComp)
    ...
    private void OnAddMarkingStep(Entity<EmberSurgeryAddMarkingStepComponent> ent, ref EmberSurgeryStepEvent args)
    {
    ...
    private void OnAddMarkingCheck(Entity<EmberSurgeryAddMarkingStepComponent> ent, ref EmberSurgeryStepCompleteCheckEvent args)
    {
        var markingCategory = MarkingCategoriesConversion.FromHumanoidVisualLayers(ent.Comp.MarkingCategory);
    ...
    private void OnRemoveMarkingStep(Entity<EmberSurgeryRemoveMarkingStepComponent> ent, ref EmberSurgeryStepEvent args)
    {

    }

    private void OnRemoveMarkingCheck(Entity<EmberSurgeryRemoveMarkingStepComponent> ent, ref EmberSurgeryStepCompleteCheckEvent args)
    {

    }

    private void OnSurgeryTargetStepChosen(Entity<EmberSurgeryTargetComponent> ent, ref EmberSurgeryStepChosenBuiMsg args)
    {
        var user = args.Actor;
    ...
    public bool TryDoSurgeryStep(EntityUid body, EntityUid targetPart, EntityUid user, EntProtoId surgeryId, EntProtoId stepId)
    {
        ...
        var speed = 1f;
        var usedEv = new EmberSurgeryToolUsedEvent(user, body);
        ...
        if (validTools?.Count > 0)
        {
            ...
            if (_net.IsServer)
            {
                ...
                {
                    if (TryComp(tool, out EmberSurgeryToolComponent? toolComp) &&
        ...
        var ev = new EmberSurgeryDoAfterEvent(surgeryId, stepId);
        var duration = GetSurgeryDuration(step, user, body, speed);

        if (TryComp(user, out EmberSurgerySpeedModifierComponent? surgerySpeedMod)
    ...
    {
        if (!TryComp(surgeryStep, out EmberSurgeryStepComponent? stepComp))
    ...
    private (Entity<EmberSurgeryComponent> Surgery, int Step)? GetNextStep(EntityUid body, EntityUid part, Entity<EmberSurgeryComponent?> surgery, List<EntityUid> requirements)
    {
    ...
    public (Entity<EmberSurgeryComponent> Surgery, int Step)? GetNextStep(EntityUid body, EntityUid part, EntityUid surgery)
    {
        return GetNextStep(body, part, surgery, new List<EntityUid>());
    }

    public bool PreviousStepsComplete(EntityUid body, EntityUid part, Entity<EmberSurgeryComponent> surgery, EntProtoId step)
    {
        ...
        if (surgery.Comp.Requirement is { } requirement)
        {
            if (GetSingleton(requirement) is not { } requiredEnt ||
                !TryComp(requiredEnt, out EmberSurgeryComponent? requiredComp) ||
    ...
    public bool CanPerformStep(EntityUid user, EntityUid body, EntityUid part,
        EntityUid step, bool doPopup, out string? popup, out EmberStepInvalidReason reason,
        out Dictionary<EntityUid, float>? validTools)
    {
        ...
        var check = new EmberSurgeryCanPerformStepEvent(user, body, GetTools(user), slot);
        RaiseLocalEvent(step, ref check);
        popup = check.Popup;
        validTools = check.ValidTools;

        if (check.Invalid != EmberStepInvalidReason.None)
        {
    ...
    public bool IsStepComplete(EntityUid body, EntityUid part, EntProtoId step, EntityUid surgery)
    {
        ...
        var ev = new EmberSurgeryStepCompleteCheckEvent(body, part, surgery);
    ...
    private bool AnyHaveComp(List<EntityUid> tools, IComponent component, out EntityUid withComp, out float speed)
    {
        ...
        {
            if (EntityManager.TryGetComponent(tool, component.GetType(), out var found) && found is IEmberSurgeryToolComponent toolComp)
            {
                withComp = tool;
